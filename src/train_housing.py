...
def load_california_housing_data() -> Tuple[pd.DataFrame, pd.Series]:
    ...
    if df is None:
        raise AttributeError('Could not extract DataFrame from fetch_california_housing result.')
...
def train_linear_regression(X_train: pd.DataFrame, y_train: pd.DataFrame, 
...
def train_decision_tree(X_train: pd.DataFrame, y_train: pd.DataFrame, 
                       ...
                       random_state: int = 42) -> Tuple[DecisionTreeRegressor, Dict[str, float]]:
    """
...
def register_best_model(best_run_id: str, best_rmse: float, model_name: str = "CaliforniaHousingBestModel") -> None:
    """
    ...
    if best_run_id:
        ...
        logger.info(f"Best model registered as '{model_name}' from run {best_run_id} with RMSE {best_rmse:.4f}")
    else:
        logger.warning("No model was registered.")


def train_all_models(X_train: pd.DataFrame, y_train: pd.Series, 
                    X_test: pd.DataFrame, y_test: pd.Series) -> Tuple[str, float]:
    """
