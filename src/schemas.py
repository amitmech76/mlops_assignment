...
class PredictionResponse(BaseModel):
    ...
    confidence: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Prediction confidence score"
    )
...
class HousingPredictionRequest(BaseModel):
    ...
    AveRooms: float = Field(
        ..., gt=0, le=20, description="Average number of rooms per household"
    )
    AveBedrms: float = Field(
        ..., gt=0, le=10, description="Average number of bedrooms per household"
    )
    Population: float = Field(..., gt=0, le=10000, description="Block group population")
    AveOccup: float = Field(
        ..., gt=0, le=50, description="Average number of household members"
    )
...
class RetrainingRequest(BaseModel):
    ...
    parameters: Optional[Dict[str, Any]] = Field(
        None, description="Model parameters for retraining"
    )
