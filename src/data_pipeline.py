...
class DataPipeline:
    ...
    def download_dataset(self, dataset_name: str) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"{dataset_name.title()} dataset downloaded: {df.shape[0]} samples, {df.shape[1]} features")
            return df
    ...
    def preprocess_iris_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"Iris preprocessing completed. Features: {list(processed_df.columns)}")
        return processed_df
    ...
    def preprocess_housing_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"Housing preprocessing completed. Features: {list(processed_df.columns)}")
        return processed_df
    ...
    def save_data(self, df_raw: pd.DataFrame, df_processed: pd.DataFrame, dataset_name: str) -> Tuple[str, str]:
        """
    ...
    def create_train_test_split(self, df: pd.DataFrame, dataset_name: str, 
    ...
    def scale_features(self, X_train: pd.DataFrame, X_test: pd.DataFrame, 
                      dataset_name: str, scaler_type: str = 'standard') -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        """
        logger.info(f"Scaling features for {dataset_name} using {scaler_type} scaler...")
    ...
    def get_data_summary(self, df: pd.DataFrame, dataset_name: str) -> Dict[str, Any]:
        ...
        if dataset_name.lower() == 'iris':
            ...
            summary['price_category_distribution'] = df['price_category'].value_counts().to_dict()
        
        return summary
    
    def run_pipeline(self, dataset_name: str, test_size: float = 0.2, 
                    random_state: int = 42, scaler_type: str = 'standard') -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Save data
            raw_path, processed_path = self.save_data(df_raw, df_processed, dataset_name)
