...
from schemas import (
    HousingPredictionRequest, IrisPredictionRequest, PredictionResponse,
    RetrainingRequest, RetrainingResponse, DataDriftRequest, DataDriftResponse,
    HealthCheckResponse, ErrorResponse
)
from prometheus_metrics import metrics_collector, get_prometheus_metrics, get_metrics_content_type
...
async def get_log_stats(hours: int = 24, model_name: Optional[str] = None):
    ...
    try:
        stats = prediction_logger.get_prediction_stats(model_name=model_name, hours=hours)
        return {
            "stats": stats,
...
async def get_retraining_status(job_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting retraining status: {str(e)}")
...
async def get_all_retraining_jobs():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting retraining jobs: {str(e)}")
...
async def detect_data_drift(request: DataDriftRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error detecting data drift: {str(e)}")
...
async def advanced_health_check():
    ...
    try:
        ...
        return HealthCheckResponse(
            status="healthy" if all([housing_model is not None, iris_model is not None, db_status]) else "unhealthy",
            models_loaded={
                "housing_model": housing_model is not None,
                "iris_model": iris_model is not None
            },
