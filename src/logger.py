...
class PredictionLogger:
    ...
    def log_prediction(self, 
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Log to file as well
                logger.info(f"Prediction logged - Model: {model_name}, Status: {status}, Time: {processing_time_ms:.2f}ms")
                
        except Exception as e:
            logger.error(f"Error logging prediction: {e}")
    
    def log_metric(self, metric_name: str, metric_value: float, model_name: Optional[str] = None):
        """Log a metric value"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
    ...
    def get_prediction_stats(self, model_name: Optional[str] = None, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                time_threshold = datetime.now().replace(hour=datetime.now().hour - hours).isoformat()
    ...
    def get_metrics(self, metric_name: Optional[str] = None, hours: int = 24) -> Dict[str, Any]:
        """Get metrics data"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
...
def log_prediction_request(model_name: str, request_data: Dict[str, Any]):
    ...
    try:
        ...
        # Log metrics
        prediction_logger.log_metric("processing_time_ms", processing_time_ms, model_name)
