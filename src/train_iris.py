...
def load_iris_data(data_path: Optional[str] = None) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    feature_cols = [col for col in df.columns if col not in ['target', 'species'] and pd.api.types.is_numeric_dtype(df[col])]
    X = df[feature_cols]
    y = df['target']
...
def evaluate_classification_model(y_true: pd.Series, y_pred: np.ndarray) -> Dict[str, float]:
    """
...
def train_logistic_regression(X_train: pd.DataFrame, y_train: pd.Series, 
                            ...
                            max_iter: int = 200) -> Tuple[LogisticRegression, Dict[str, float]]:
    """
...
def train_random_forest(X_train: pd.DataFrame, y_train: pd.Series, 
                       X_test: pd.DataFrame, y_test: pd.Series,
                       n_estimators: int = 100) -> Tuple[RandomForestClassifier, Dict[str, float]]:
    """
...
def train_support_vector_machine(X_train: pd.DataFrame, y_train: pd.Series, 
                               X_test: pd.DataFrame, y_test: pd.Series) -> Tuple[SVC, Dict[str, float]]:
    """
...
def log_model_with_mlflow(model: Any, model_name: str, metrics: Dict[str, float], 
                         X_test: pd.DataFrame, run_name: str) -> str:
    """
...
def register_best_model(best_run_id: Optional[str], best_accuracy: float, model_name: str = "IrisBestModel") -> None:
    """
    ...
    if best_run_id:
        ...
        logger.info(f"Best model registered as '{model_name}' from run {best_run_id} with accuracy {best_accuracy:.4f}")
    else:
...
def train_all_models(X_train: pd.DataFrame, y_train: pd.Series, 
                    X_test: pd.DataFrame, y_test: pd.Series) -> Tuple[Optional[str], float]:
    """
    ...
    for model_name, train_func in models_to_train:
        try:
            ...
            run_id = log_model_with_mlflow(model, model_name, metrics, X_test, model_name)
...
def print_classification_report(y_test: pd.Series, y_pred: np.ndarray, model_name: str) -> None:
    """
