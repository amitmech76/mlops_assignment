...
class ModelEvaluator:
    ...
    def __init__(self, mlruns_path: str = "mlruns", metrics_path: str = "metrics", plots_path: str = "plots"):
    ...
    def get_latest_models(self) -> Dict[str, Any]:
        ...
        try:
            ...
            housing_model = mlflow.pyfunc.load_model("models:/CaliforniaHousingBestModel/latest")
            models_info["housing"] = {
                "model": housing_model,
                "type": "regression",
    ...
    def evaluate_regression_model(self, model, data: pd.DataFrame, model_name: str) -> Dict[str, float]:
        """Evaluate regression model"""
        # Prepare data
        target_col = "MedHouseVal" if "MedHouseVal" in data.columns else data.columns[-1]
        ...
        logger.info(f"{model_name} Regression Metrics: RMSE={metrics['rmse']:.4f}, R²={metrics['r2_score']:.4f}")
        return metrics
    
    def evaluate_classification_model(self, model, data: pd.DataFrame, model_name: str) -> Dict[str, Any]:
        ...
        logger.info(f"{model_name} Classification Metrics: Accuracy={accuracy:.4f}, F1={f1:.4f}")
        return metrics
    ...
    def create_performance_plots(self, all_metrics: Dict[str, Dict]):
        ...
        if "housing" in all_metrics:
            ...
            bars1 = ax1.bar(metrics_names, metrics_values, color=['#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A'])
        ...
        if "iris" in all_metrics:
            ...
            bars2 = ax2.bar(metrics_names, metrics_values, color=['#98D8C8', '#F7DC6F', '#BB8FCE', '#85C1E9'])
        ...
        if models_data:
            ...
            bars4 = ax4.bar(df_comparison['Model'], df_comparison['Primary Metric'],
                           color=['#FF6B6B', '#4ECDC4'])
