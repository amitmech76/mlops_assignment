...
def download_housing_data() -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(f"California Housing dataset downloaded successfully: {df.shape[0]} samples, {df.shape[1]} features")
        return df
...
def validate_housing_data(df: pd.DataFrame) -> bool:
    ...
    positive_features = ['MedInc', 'HouseAge', 'AveRooms', 'AveBedrms', 'Population', 'AveOccup', 'MedHouseVal']
...
def preprocess_housing_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    numeric_columns = ['MedInc', 'HouseAge', 'AveRooms', 'AveBedrms', 'Population', 'AveOccup', 'MedHouseVal']
    ...
    processed_df['total_bedrooms'] = processed_df['AveBedrms'] * processed_df['Population']
    processed_df['room_to_bedroom_ratio'] = processed_df['AveRooms'] / processed_df['AveBedrms']
    processed_df['income_per_person'] = processed_df['MedInc'] / processed_df['AveOccup']
    processed_df['house_age_squared'] = processed_df['HouseAge'] ** 2
    
    # Handle any infinite values from division
    processed_df['room_to_bedroom_ratio'] = processed_df['room_to_bedroom_ratio'].replace([np.inf, -np.inf], np.nan)
    processed_df['room_to_bedroom_ratio'] = processed_df['room_to_bedroom_ratio'].fillna(processed_df['room_to_bedroom_ratio'].mean())
    
    processed_df['income_per_person'] = processed_df['income_per_person'].replace([np.inf, -np.inf], np.nan)
    processed_df['income_per_person'] = processed_df['income_per_person'].fillna(processed_df['income_per_person'].mean())
    
    # Add geographic features
    processed_df['distance_from_coast'] = np.sqrt(processed_df['Latitude']**2 + processed_df['Longitude']**2)
    processed_df['lat_long_interaction'] = processed_df['Latitude'] * processed_df['Longitude']
    
    # Add statistical features
    for column in ['MedInc', 'HouseAge', 'AveRooms', 'AveBedrms']:
        processed_df[f'{column}_zscore'] = (processed_df[column] - processed_df[column].mean()) / processed_df[column].std()
...
def save_housing_data(df_raw: pd.DataFrame, df_processed: pd.DataFrame, 
...
def create_housing_train_test_split(df: pd.DataFrame, test_size: float = 0.2, 
...
def get_housing_data_summary(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    summary = {
        ...
        'price_category_distribution': df['price_category'].value_counts().to_dict() if 'price_category' in df.columns else {},
...
def scale_housing_features(X_train: pd.DataFrame, X_test: pd.DataFrame, 
...
def main():
    ...
    """
    try:
        logger.info("Starting California Housing data loading and preprocessing pipeline...")
        ...
        logger.info(f"  Target range: ${summary['target_summary']['min']:.2f} - ${summary['target_summary']['max']:.2f}")
        ...
        logger.info("California Housing data loading and preprocessing completed successfully!")
