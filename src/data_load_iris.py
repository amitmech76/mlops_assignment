...
def download_iris_data() -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(
            f"Iris dataset downloaded successfully: {df.shape[0]} samples, {df.shape[1]} features"
        )
        return df
...
def preprocess_iris_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    processed_df["sepal_length_zscore"] = (
        processed_df["sepal_length"] - processed_df["sepal_length"].mean()
    ) / processed_df["sepal_length"].std()
    processed_df["petal_length_zscore"] = (
        processed_df["petal_length"] - processed_df["petal_length"].mean()
    ) / processed_df["petal_length"].std()
...
def create_iris_train_test_split(
